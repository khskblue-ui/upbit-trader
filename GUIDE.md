...
6. [매도 알고리즘: 3단계 안전 청산](#6-매도-알고리즘-3단계-안전-청산)
...
└── data/
    ...
    └── positions.json      ← ⭐ 열린 포지션 스냅샷 (재시작 시 자동 복구)
```
...
[2단계] 데이터 정규화
   ...
   → 전략이 쓸 수 있게 영어로 바꿈 (high, low, open, close, volume)
         ↓
[3단계] 전략 판단 (trend_filtered_breakout / TFVB)
         ↓
   스크린1: EMA(20) > EMA(60) → ✅ 상승 추세 확인
   스크린2: RSI(14) = 58.3 ∈ [45, 70] → ✅ 건강한 모멘텀
   스크린3: ATR(14) = 1,800,000원 (14일 평균 변동폭)
   오늘 시가:  93,500,000원
   목표가:     93,500,000 + 1,800,000 × 0.4 = 94,220,000원
   현재가:     94,600,000원

   현재가 94,600,000 >= 목표가 94,220,000 → ✅ BUY 신호!
         ↓
[4단계] 리스크 검사 (4가지 규칙)
         ↓
   규칙1: 한 코인 비중 20% 이하? → ✅
...
```
조건: 현재가 ≥ 오늘 시가 + ATR(14일 평균) × K값(0.4)

ATR(14) = 최근 14일간의 일평균 변동폭 (단일 세션 이상치에 강건)
```

> **왜 어제 변동폭 대신 ATR을 쓰나요?** 어제 변동폭만 쓰면 전날 이상 급등/급락 때
> 목표가가 비정상적으로 높거나 낮아져 잘못된 진입이 발생합니다(휩소).
> ATR은 14일 평균이므로 한 세션의 노이즈에 영향을 받지 않습니다.

스크린 1, 2를 통과한 상태에서만 이 조건을 검사합니다.

**실제 이더리움 예시:**
```
최근 14일 ATR: 120,000원 (14일 평균 변동폭)

오늘:
  시가: 2,980,000원
  목표가: 2,980,000 + 120,000 × 0.4 = 3,028,000원

현재가 3,050,000원 → 목표가 초과 → ✅ 스크린 3 통과!
현재가 3,010,000원 → 목표가 미달 → ❌ 관망
```
...
| **시간 기준** | 일봉 (하루 단위 판단) |
| **매도 알고리즘** | 3단계 자동 청산: HARD_STOP(5%) → ATR_TRAIL(2×ATR) → TIME_EXIT(5세션) |
| **K값** | 0.4 (실시간 변경: `/k 0.4`) |
| **1회 최대 투자** | 자본의 20% |
...
TFVB는 단순한 "다음 날 시가 청산" 대신 **3개의 독립적인 청산 레이어**로 포지션을 관리합니다. 시장 급락, 추세 반전, 장기 횡보 모두에 대응합니다.

```
포지션 보유 중 (매 사이클 검사 순서)
         ↓
   [Layer 1] HARD_STOP: 현재가 ≤ 진입가 × (1 - 0.05)
             → 5% 갭다운 즉시 매도 (리스크 엔진 거부 무시)
         ↓
   [Layer 2] ATR_TRAIL: 현재가 ≤ 트레일링 스탑
             → ATR × 2 거리를 유지하며 상승 추종
             → 09:00 KST 세션 교체 시에만 상향 조정 (노이즈 방지)
         ↓
   [Layer 3] TIME_EXIT: 보유 세션 수 ≥ 최대 보유일(5)
             → 데드머니 방지, 강제 청산
```

---

### Layer 1: HARD_STOP — 즉각 손절

```
hard_stop = 진입가 × (1 - hard_stop_pct)    # 기본 5%
```

- 주요 이벤트·갭다운 등 **급락 시 무조건 실행** (리스크 엔진 REJECT도 무시)
- 텔레그램 알림 사유: `HARD_STOP(2,850,000): 급락/갭다운 — 진입가 대비 -5.2%`

---

### Layer 2: ATR_TRAIL — 추세 추종 트레일링 스탑

```
trailing_stop = 진입가 - atr_trail_mult × ATR(14)  # 초기
               (이후 세션마다 상향 조정만, 하락 안 함)
```

- **상승 시**: 이익을 잠그면서 스탑이 올라감
- **하락 시**: 스탑이 트리거되어 이익 실현 또는 손실 제한
- 09:00 KST 기준 세션이 바뀔 때만 업데이트 (분 단위 노이즈 탈출 방지)

---

### Layer 3: TIME_EXIT — 시간 청산

```
sessions_held = 현재 세션 날짜 - 매수 세션 날짜  (09:00 KST 기준)
if sessions_held >= max_hold_days:  → 강제 청산
```

- **Upbit 세션 기준**: 09:00 KST (자정이 아님)
  - 예: 23:55에 매수 → 다음 날 09:00에 새 세션 시작 → 1세션 경과
- 기본 5세션 이후 "데드머니(방치 포지션)" 강제 청산
...
```yaml
params:
  atr_trail_mult: 2.0    ← Layer 2 트레일링 배수 (낮출수록 타이트)
  hard_stop_pct: 0.05    ← Layer 1 손절 비율 (5%)
  max_hold_days: 5       ← Layer 3 최대 보유 세션 수
```

---

### 포지션 영속화 (`data/positions.json`)

봇을 재시작해도 열린 포지션 정보(진입가·ATR·스탑 레벨·매수 세션)가 유지됩니다.

- 매수/매도/트레일링 업데이트마다 자동 저장
- 재시작 시 `data/positions.json` 자동 로드
- 파일이 없거나 손상되면 **Orphan Recovery** 작동:
  보수적 기본값으로 포지션 레코드를 재생성, 다음 사이클에 TIME_EXIT 유발
...
```yaml
strategies:
  - name: trend_filtered_breakout  ← ⭐ 유일한 전략 (TFVB)
    enabled: true
    markets:
    ...
    params:
      ...
      # ── 매도 파라미터 (3단계 청산) ────────────────────────────────
      atr_trail_mult: 2.0          ← 트레일링 스탑 ATR 배수 (낮을수록 타이트)
      hard_stop_pct: 0.05          ← 즉시 손절 기준 5% (갭다운 대응)
      max_hold_days: 5             ← 최대 보유 세션 수 (09:00 KST 기준)
```

> **📌 `base_capital` 설정 불필요**: 실제 계좌 잔액이 자동으로 포지션 사이징에 반영됩니다.
> (paper 모드: 가상 잔액 기준 / live 모드: 업비트 실제 KRW 잔액 기준)
...
# 매도 파라미터 (3단계 청산)
atr_trail_mult — 트레일링 스탑 ATR 배수 (현재 2.0)
hard_stop_pct  — 즉시 손절 비율 (현재 0.05 = 5%)
max_hold_days  — 최대 보유 세션 수 (현재 5)
```
...
```
🔵 [매도 체결]   ← ATR_TRAIL 트리거 (수익)
마켓: KRW-ETH
가격: 3,100,000 KRW
수량: 0.054934
손익: +3,244 KRW (+1.97%)
전략: trend_filtered_breakout
사유: ATR_TRAIL(3,020,000): 추세 반전 — 트레일링 스탑 터치

🔴 [매도 체결]   ← HARD_STOP 트리거 (손실)
마켓: KRW-ETH
가격: 3,040,000 KRW
수량: 0.054934
손익: -8,760 KRW (-5.23%)
전략: trend_filtered_breakout
사유: HARD_STOP(3,040,000): 급락/갭다운 — 진입가 대비 -5.2%

🔵 [매도 체결]   ← TIME_EXIT (5세션 보유 후 청산)
사유: TIME_EXIT: 5세션 보유 (최대 5세션, 09:00 KST 기준)
```
...
**A.** 3단계 매도 알고리즘이 항상 작동하고 있습니다. 다음 조건 중 하나가 충족되면 자동 청산됩니다.

| 조건 | 설명 | 로그 키워드 |
|------|------|-------------|
| HARD_STOP | 진입가 대비 5% 하락 | `HARD_STOP(...)` |
| ATR_TRAIL | 트레일링 스탑 터치 | `ATR_TRAIL(...)` |
| TIME_EXIT | 5세션(09:00 KST 기준) 보유 후 강제 청산 | `TIME_EXIT(...)` |

매수 후 5세션이 지나도 매도가 없다면 로그를 확인하세요:
```bash
sudo journalctl -u upbit-trader --since today | grep -E "SELL|TIME_EXIT|HARD_STOP|ATR_TRAIL"
```

매도가 전혀 발생하지 않는다면 포지션 파일(`data/positions.json`)이 있는지 확인하세요.
...
/set trend_filtered_breakout atr_trail_mult 1.5       트레일링 스탑 배수 변경
/set trend_filtered_breakout hard_stop_pct 0.03       즉시 손절 비율 변경 (3%)
/set trend_filtered_breakout max_hold_days 7          최대 보유일 변경
/enable trend_filtered_breakout                       전략 활성화
/disable trend_filtered_breakout                      전략 비활성화
...
> 문서 최종 업데이트: 2026-03-04 (3단계 매도 알고리즘, 포지션 영속화, ATR 돌파 반영)
